...
import { nutrientGroups, aafcoProfiles, allNutrients } from '../constants';
...
export default function RecipeSolver() {
        ...
        ingredients.forEach(ing => {
            ...
            if (amt > 0) {
                ...
                data.push({
                    ...
                    'Unit Price (VND)': price.toLocaleString(),
                    'Cost (VND)': Math.round(amt * price).toLocaleString()
                });
            }
        });

        const totalCost = currentResult.result;
        data.push({ Code: 'TOTAL', Ingredient: '', Group: '', 'Amount (kg)': recipe.targetWeight.toFixed(2), 'Ratio (%)': '100%', 'Unit Price (VND)': '', 'Cost (VND)': Math.round(totalCost).toLocaleString() });
    ...
    const displayCost = result && result.feasible ? Math.round(result.result).toLocaleString() : Math.round(manualCost).toLocaleString();
    const displayPricePerKg = result && result.feasible ? Math.round(result.result / recipe.targetWeight).toLocaleString() : Math.round(manualCost / recipe.targetWeight).toLocaleString();
    ...
    const handleCopy = () => {
        const newRecipe = { ...recipe, name: recipe.name + ' (Copy)' };
        delete newRecipe.id;
        ...
        <div className="spreadsheet-layout animate-fade-in" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 80px)', background: '#fff', color: '#333', overflow: 'hidden', margin: '-2rem', fontFamily: 'Arial, sans-serif' }}>
            ...
            <div style={{ padding: '8px 16px', borderBottom: '1px solid #d1d5db', fontSize: '12px', display: 'flex', justifyContent: 'space-between', background: '#f8f9fa' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'min-content min-content min-content min-content', gap: '8px 40px', alignItems: 'center', whiteSpace: 'nowrap' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '180px' }}><strong>Price:</strong> <strong>{displayPricePerKg} VND/kg</strong></div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', width: '180px' }}><strong>Cost:</strong> <span>{displayCost} VND</span></div>
                                ...
                                {(recipe.addedIngredients || []).map((code, idx) => {
                                    ...
                                    return (
                                        <tr key={ing.code} style={{ background: isUsed ? (idx % 2 === 0 ? '#fff' : '#f8f9fa') : '#f1f5f9', opacity: isUsed ? 1 : 0.6, borderBottom: '1px solid #f3f4f6' }}>
                                            ...
                                            <td style={{ padding: '4px 8px', borderRight: '1px solid #e5e7eb', color: '#4b5563' }}>{price.toLocaleString()}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
