...
import { Plus, Edit2, Trash2, X, Check, Search, Copy } from 'lucide-react';
...
export default function Ingredients() {
    ...
    const calculateDerivedNutrients = (nutrients, triggerKey = null) => {
        ...
        const aminoKeys = ['Methionine', 'Cystine', 'Phenylalanine', 'Tyrosine'];
        if (triggerKey === null || aminoKeys.includes(triggerKey)) {
            const met = num('Methionine');
            const cys = num('Cystine');
            const phe = num('Phenylalanine');
            const tyr = num('Tyrosine');
        ...
        // Recalculate Omega 6/3 Ratio
        const omegaKeys = ['Omega_6', 'Omega_3'];
        if (triggerKey === null || omegaKeys.includes(triggerKey)) {
            const o6 = num('Omega_6');
            const o3 = num('Omega_3');
            const ratio = o3 > 0 ? o6 / o3 : 0;
            newNutrients['Omega_6_3_Ratio'] = String(Math.round(ratio * 100) / 100);
        }
    ...
    const handleCopy = (ing) => {
        setEditingIng(null);
        // Convert numbers to strings for form inputs while keeping exact values
        const stringNutrients = {};
        Object.keys(ing.nutrients).forEach(key => {
            stringNutrients[key] = String(ing.nutrients[key]);
        });
                                ...
                                <tr key={ing.code}>
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                                            <button className="btn btn-secondary" style={{ padding: '0.4rem' }} title="Copy" onClick={() => handleCopy(ing)}><Copy size={16} /></button>
                                            <button className="btn btn-secondary" style={{ padding: '0.4rem' }} title="Edit" onClick={() => handleOpen(ing)}><Edit2 size={16} /></button>
                                            <button className="btn btn-danger" style={{ padding: '0.4rem' }} title="Delete" onClick={() => deleteIngredient(ing.code)}><Trash2 size={16} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
